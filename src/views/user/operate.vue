<template>
  <div class="operate-management">
      ...
      <el-table :data="tableData" border style="width: 100%">
        ...
        <el-table-column fixed="right" label="Operations" min-width="300">
          <template #default="scope">
            ...
            <el-button link type="primary" size="small" @click="onSetAuth(scope.$index, scope.row)">
              权限分配
            </el-button>
          </template>
        </el-table-column>
    ...
    <set-auth-dialog
      ...
      :backfillData="setAuthBackfillData"
      @submit-after="submitAfter"
    >
    </set-auth-dialog>
  </div>
...
import type { OperateUserInfo } from '@/types/operate'
import AddDialog from './components/operate/addDialog.vue'
import SetAuthDialog from './components/operate/setAuthDialog.vue'
...
const tableData: OperateUserInfo[] = computed(() => operateStore.table.tableData)

const queryTableData = async () => {
  const { pageSize, pageNum } = operateStore.table
  await operateStore.operateQueryReq({
    pageSize,
    pageNum,
    ...searchForm.value,
  })
...
const operateDetailReq = async ({ operId }) => {
  await operateStore.operateDetailReq({ operId })
  const detailData = operateStore.detailData
  backfillData.value = {
...
// 详情方法
const onDetail = async (index: number, row: OperateUserInfo) => {
  await operateDetailReq({ operId: row.operId })
  dialogType.value = 'detail'
}

// 修改方法
const onEdit = async (index: number, row: OperateUserInfo) => {
  await operateDetailReq({ operId: row.operId })
  dialogType.value = 'edit'
}

// 删除方法
const onDelete = async (index: number, row: OperateUserInfo) => {
  console.log('删除', 'index::', index, 'row:', row)
  await operateStore.operateRemoveReq({ operId: row.operId })
  await queryTableData()
}

// 重置密码
const onResetPassword = async (index: number, row: OperateUserInfo) => {
  console.log('用户查询', 'index::', index, 'row:', row)
  await operateStore.operateResetPasswordReq({ operId: row.operId })
  await queryTableData()
}

// 权限分配
const onSetAuth = async (index: number, row: OperateUserInfo) => {
  console.log('权限分配', 'index::', index, 'row:', row)
  await operateStore.operateDetailReq({ operId: row.operId })
  const detailData = operateStore.detailData
  setAuthBackfillData.value = {
    operId: detailData.operId,
    deptId: detailData.deptId,
    operCode: detailData.operCode,
    operName: detailData.operName,
...
const onDownload = async () => {
  console.log('下载')
  await operateStore.operateDownloadReq()
}
</script>
